...
export async function uploadPhoto(file:File,operation:string):Promise<Photo>{ready();if(file.size<1||file.size>MAX_PHOTO_BYTES)throw new AppError(400,"התמונה גדולה מדי. נסה לבחור אותה שוב כדי לכווץ אותה.");const buffer=await file.arrayBuffer(),bytes=new Uint8Array(buffer),mime=mimeOf(bytes);if(!mime)throw new AppError(400,"אפשר לצרף תמונות JPG, PNG או WebP בלבד.");const id=crypto.randomUUID(),extension=mime==="image/jpeg"?"jpg":mime==="image/png"?"png":"webp";const blob=await put(`photos/${operation}/${id}.${extension}`,Buffer.from(buffer),{access:"public",addRandomSuffix:false,contentType:mime});return {id,path:blob.url,name:file.name.slice(0,160),mime,size:file.size};}
